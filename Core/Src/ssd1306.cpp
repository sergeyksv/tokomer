...
void SSD1306::start_horizontal_scroll(unsigned char direction, unsigned char start, unsigned char end, unsigned char interval) 
{
    ...
    switch (interval) {
        ...
        case 255: _send_command(0x03); break; // 011b
        default:
...
void SSD1306::puts(const char *s) {
...
{
    HAL_GPIO_WritePin(DISPDC_GPIO_Port, DISPDC_Pin, GPIO_PIN_RESET);
    HAL_SPI_Transmit(&hspi1,&code,1,100);
}
